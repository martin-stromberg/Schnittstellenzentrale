// Generalized asset-repair mechanism (ci-target-schema.md section 4.10), adopted from
// FinanceManager's scripts/resolve-release-version.mjs (the project-wide template, already
// stripped of FinanceManager-specific names) and adapted for Schnittstellenzentrale's
// single-asset (win-x64 only) release shape - see .github/actions/build-and-package.
//
// This replaces the previous, much simpler resolve-release-version.mjs, which was only a
// semantic-release dry-run probe writing should_release/version/tag outputs. It did not
// implement asset-repair or manual-tag releases at all.
//
...
export function expectedReleaseAssetNames(version) {
  return [releaseAssetName(version, "win-x64"), "update.json"];
}
...
function runSemanticReleaseDryRun() {
  ...
  const result = spawnSync(process.execPath, [semanticReleaseBinary, "--dry-run"], {
    encoding: "utf8",
    // RESOLVE_DRY_RUN selects release.config.cjs's lightweight dryRunPlugins (commit-analyzer
